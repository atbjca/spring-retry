...
> **当前是 Nexus 已验证的 SNAPSHOT，不是已发布的安全 RELEASE。** 项目已完成 CVE-2026-41710 源码修复、Java 8/Spring Framework 5.3.x 双矩阵、覆盖率、SNAPSHOT 部署、远端四件套及 JDK 8 消费者验证；正式 RELEASE、RELEASE 远端复验和 Git tag 仍未完成。CVE 主状态按要求继续为“修复中”，请勿把 SNAPSHOT 当作生产 RELEASE。
...
| 开发 GAV | `cn.bjca.footstone.bpring.retry:bjca-footstone-bpring-retry:1.3.4-nes.patch.1-SNAPSHOT` |
| 计划 RELEASE GAV | `cn.bjca.footstone.bpring.retry:bjca-footstone-bpring-retry:1.3.4-nes.patch.1`，尚未发布 |
...
| Nexus SNAPSHOT | 四件套已部署并从隔离 Maven repository 重新解析，摘要一致 |

Spring Framework 4.x、Java 6/7 和 Spring Framework 6.x API 不在支持范围。

## 当前安全状态

CVE-2026-41710 主状态为“修复中”，阶段为“源码与 Nexus SNAPSHOT 已验证，待 RELEASE”。普通缓存现为有界访问顺序 LRU，断路器使用独立严格容量缓存；目标 RELEASE 尚未生成或远端验证。
...
可通过 `JAVA8_HOME`、`DEV_ROOT`、`MAVEN` 和 `MAVEN_FLAGS` 覆盖本地路径。`make install-local` 只安装 SNAPSHOT 到本地 Maven repository；`make deploy` 根据版本自动选择 snapshots/releases，RELEASE 必须显式设置 `ALLOW_RELEASE_DEPLOY=true`。Makefile 不提供 Git tag/push，Gradle 仍只用于临时消费者，不是本仓库构建系统。

当前 SNAPSHOT 的已授权部署命令为：

```bash
make clean deploy
```

RELEASE 入口只可在独立 release change 完成版本冻结、目标不存在检查并再次取得部署授权后使用：
...
| 文档 | 用途 |
| --- | --- |
| [快速入门](doc/QUICK_START.md) | Nexus SNAPSHOT、Maven/Gradle 消费、部署门禁、最小用例和依赖去重 |
| [用户手册](doc/USER_MANUAL.md) | 声明式/命令式、无状态/有状态、listener、断路器和缓存行为 |
| [维护要求](doc/REQUIREMENTS.md) | OpenSpec、审批、TDD、覆盖率、影响分析和安全设计 |
| [GAV 映射](doc/GAV_MAPPING.md) | 当前 SNAPSHOT、计划 RELEASE 与两套 Framework 坐标 |
...
| [发布说明](doc/RELEASE_NOTES.md) | `1.3.4-nes.patch.1` 的未发布状态、已完成基线和剩余阻断项 |
