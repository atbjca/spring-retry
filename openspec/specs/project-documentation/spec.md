...
`doc/USER_MANUAL.md` MUST 覆盖声明式/命令式重试、无状态/有状态重试、RetryListener、断路器、RetryContextCache、容量策略、缓存 Bean 命名、异常和从官方 GAV 迁移的注意事项。完成 CVE-2026-41710 remediation 后，手册 MUST 说明普通 map/soft-reference cache 的默认访问顺序 LRU、已有 key 更新、严格容量构造方式、普通/断路器双缓存路由、两个约定 Bean 名、唯一 Bean 回退规则、WARN 监控和状态可能提前淘汰的残余风险。

#### Scenario: CVE 缓存修复改变默认行为

- **WHEN** remediation 将普通默认缓存由满容量抛异常改为有界 LRU，并隔离断路器缓存
- **THEN** User Manual MUST 在同一 change 中更新默认值、Java/Maven 配置示例、两个命名 Bean、可用性收益、状态淘汰权衡、fail-fast 选择和未发布状态

#### Scenario: 用户保留严格容量行为

- **WHEN** 用户不接受普通缓存自动淘汰重试状态
- **THEN** User Manual MUST 提供 `removeEldestEntries=false` 或自定义 `RetryContextCache` 示例，并说明容量满后新 key 将继续 fail-fast

#### Scenario: 用户从单个自定义缓存迁移

- **WHEN** 旧应用曾依赖唯一 `RetryContextCache` Bean 同时影响普通重试和断路器
- **THEN** User Manual MUST 说明升级后唯一 Bean 只回退给普通重试，并提供 `retryContextCache` 与 `circuitBreakerRetryContextCache` 双 Bean 迁移示例
